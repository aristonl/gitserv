...
int initRepos() {
    ...
    if (dir == NULL) {
        printf("Cannot open /srv/git. Make sure you are running with sudo or as root.\n");
        return -1;
    }
    ...
    while ((ent = readdir(dir)) != NULL) {
        ...
        if (ent->d_type == DT_DIR) {
            ...
            if (chdir(userFolder.c_str()) != 0) {
                printf("Cannot change to user folder: %s. Make sure you are running with sudo or as root.\n", userFolder.c_str());
                return -1;
            }

            DIR* userDir = opendir(userFolder.c_str());
            if (userDir == NULL) {
                printf("Cannot open user folder: %s. Make sure you are running with sudo or as root.\n", userFolder.c_str());
                return -1;
            }

            struct dirent* userEnt;
            while ((userEnt = readdir(userDir)) != NULL) {
                if (strcmp(userEnt->d_name, ".") == 0 || strcmp(userEnt->d_name, "..") == 0) {
                    continue;
                }

                // FIXME: This currently doesn't iterate thru *every* directory/repository in a user's folder.
                if (userEnt->d_type == DT_DIR) {
                    git_repository* repository;
                    std::string repoFolder = userFolder + "/" + userEnt->d_name;
                    printf("Found repo folder: %s\n", repoFolder.c_str());
                    int error = git_repository_open(&repository, repoFolder.c_str());
                    if (error < 0) {
